# Cleaning up what is not used

You will notice that after many commands back and forth, a lot of things are left
behind, such as stopped containers, non-"functional" images and so on. To clean
up this mess, run the command below:

```
docker system prune
```

You will get something like:

```
rafael @ nazgul ~
└─ $ ▶ docker system prune
WARNING! This will remove:
        - all stopped containers
        - all networks not used by at least one container
        - all dangling images
        - all build cache
Are you sure you want to continue? [y/N] y
Deleted Containers:


Deleted Images:
deleted: sha256:2d92e5d5adbe5e207b36d93bd14d8b75919d4a118c86f4b137a9c46481b98fef
deleted: sha256:8ae3118b00c274114fd91dc96628b845e0ff4722717bfdf343c2207bad1ec01f

Total reclaimed space: 70.62MB
```

By default `docker system prune` does **not** remove volumes, nor images that are
merely untagged because they are old. For a more aggressive cleanup:

```
docker system prune -a --volumes
```

- `-a` also removes every image with no associated container (not just the
  *dangling* ones).
- `--volumes` removes the anonymous volumes that are not in use — be careful,
  data lives here.

You can also clean one type of object at a time:

```
docker container prune
docker image prune -a
docker volume prune
docker builder prune       # BuildKit build cache
```
